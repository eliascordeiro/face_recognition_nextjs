...
export default function OperatorEmployeesPage() {
  ...
  const canManage = hasPerm(auth, 'employees.manage')
  const canFace = hasPerm(auth, 'employees.face')
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((emp) => (
            <div key={emp.id} className="bg-slate-800 border border-slate-700 rounded-xl p-4 flex flex-col hover:border-slate-600 transition-colors">
              <div className="flex items-start gap-3">
                {emp.thumbnail ? (
                  <img src={emp.thumbnail} alt={emp.name} className="w-14 h-14 rounded-full object-cover border-2 border-slate-700 flex-shrink-0" />
                ) : (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className={`inline-flex items-center gap-1 mt-1.5 text-[11px] px-2 py-0.5 rounded-full border ${
                    emp.has_face ? 'bg-emerald-900/40 border-emerald-700 text-emerald-300' : 'bg-amber-900/40 border-amber-700 text-amber-300'
                  }`}>
